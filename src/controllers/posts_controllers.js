...
try {
    ...
    const result = await pool.query("UPDATE posts SET title = IFNULL(?,  title) , description = IFNULL(?,  description) , img = IFNULL(?,  img), ,materials = IFNULL(?,  img), stepbystep = IFNULL(?,  img) WHERE idposts = ?", [title, description, img,,materials, stepbystep, idposts])
